...
5. **Reference `toyota-way-agents` skill in AGENTS.md and CLAUDE.md once available** — the Toyota Way × AI agents skill is being designed in [`andrej-karpathy-skills`](https://github.com/chaz-clark/andrej-karpathy-skills) (handoff doc in fork). Once it lands in `make-ai-agents` and gets subtree'd here, AGENTS.md should cite it as the recommended companion to `karpathy-guidelines` for non-technical users — produces a quality-disciplined, plan-before-execute, trust-buildable agent.
6. **Convert `agents/make_agent*` files from copied → subtree relationship** — `make_agent.md/.json` and `make_agent_qc.md/.json` were copied (not subtree'd) from the upstream `make-ai-agents` repo. They will drift over time. Set up `git subtree` so updates propagate cleanly. Required prep: confirm `make-ai-agents` URL, branch, and which subdirectory holds the canonical files; resolve any divergence between local and upstream versions before the subtree add.
